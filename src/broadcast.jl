...
databroadcast(f::Function, As...) = broadcast!(f, DataArray(promote_eltype(As...), broadcastshape(As...)), As...)
pdabroadcast(f::Function, As...) = broadcast!(f, PooledDataArray(promote_eltype(As...), broadcastshape(As...)), As...)
...
@da_broadcast_vararg (.+)(As...) = broadcast!(+, DataArray(eltype_plus(As...), broadcastshape(As...)), As...)
@da_broadcast_binary (.-)(A, B) =
    broadcast!(-, DataArray(promote_op(@functorize(-), eltype(A), eltype(B)),
                            broadcastshape(A,B)), A, B)
@da_broadcast_binary (./)(A, B) =
    broadcast!(/, DataArray(promote_op(@functorize(/), eltype(A), eltype(B)),
                            broadcastshape(A, B)), A, B)
@da_broadcast_binary (.\)(A, B) =
...
(.+)(As::PooledDataArray...) =
    broadcast!(+, PooledDataArray(eltype_plus(As...), broadcastshape(As...)), As...)
...
for (sf, vf) in zip(scalar_comparison_operators, array_comparison_operators)
    @eval begin
        ...
        $(vf)(A::(@compat Union{PooledDataArray{Bool},DataArray{Bool}}), B::(@compat Union{PooledDataArray{Bool},DataArray{Bool}})) =
            broadcast!($sf, DataArray(Bool, broadcastshape(A, B)), A, B)
        ...
        @da_broadcast_binary $(vf)(A, B) = broadcast!($sf, DataArray(Bool, broadcastshape(A, B)), A, B)
    end
end
